...
(function()
{
    ...
    attributeChanges)
  {
    ...
    if (attributeChanges.hasChanged("_fragmentLoaded"))
    {
      ...
      {
        // This is the use case where the markNodeForUpdate call was made for the fragment DFD being
  ...
  fragmentHandler.prototype._createChildrenFromFragment = function(amxNode)
  {
    ...
    if (fragmentTagDfd.state() == "resolved")
    {
      ...
      if (isFinestLoggingEnabled)
      {
        adf.mf.log.AMX.logp(adf.mf.log.level.FINEST,
          "amx:fragment", "_createChildrenFromFragment",
          "Deferred object is resolved. Calling _handleLoadedFragment (URI: " + fragmentUri +
    ...
    {
      // The fragment file has failed to be loaded
      if (isFinestLoggingEnabled)
      {
        adf.mf.log.AMX.logp(adf.mf.log.level.FINEST,
          "amx:fragment", "_createChildrenFromFragment",
          "Failed to load the fragment file (URI: " + fragmentUri +
          ", AmxNode: " + amxNode.getId() + ")");
      }
    ...
    fragmentTagDfd)
  {
    ...
    // process it and create the children. Use the .done method to be able to access
    // the AmxTag for the fragment file.
    var th = this;
    fragmentTagDfd
      .done(
        function(fragmentTag)
        {
          if (isFinestLoggingEnabled)
          {
            adf.mf.log.AMX.logp(adf.mf.log.level.FINEST,
              "amx:fragment", "_handleLoadedFragment",
              "Fragment tag DFD callback invoked for node: " + amxNode.getId());
          }
          ...
          try
          {
            ...
            if (!fragmentState["fragmentLoading"])
            {
              if (isFinestLoggingEnabled)
              {
                adf.mf.log.AMX.logp(adf.mf.log.level.FINEST,
                  ...
                  "The fragment tag DFD has completed, but the state is no longer loading. The " +
                  " markNodeForUpdate method will not be invoked for node: " +
                  amxNode.getId());
              }
            }
            else
            {
              if (isFinestLoggingEnabled)
              {
                adf.mf.log.AMX.logp(adf.mf.log.level.FINEST,
                  "amx:fragment", "_loadFragment",
                  "The fragment tag DFD has completed. Calling markNodeForUpdate for node: " +
                  amxNode.getId());
              }
