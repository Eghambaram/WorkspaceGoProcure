...
(function()
{
      ...
      for (i = 0; i < maxRows && iter.hasNext(); ++i)
      {
        ...
        if (children.length > 0)
        {
          ...
          previousListItemAmxNodeElement = this._renderStampedItem(
            amxNode, innerListElement, listItemAmxNode, previousListItemAmxNode,
        ...
        iter.getAvailableCount() < maxRows)
      {
        ...
        function(req, resp)
        {
          adf.mf.log.logInfoResource("AMXInfoBundle", adf.mf.log.level.SEVERE,
            "createChildrenNodes", "MSG_ITERATOR_FIRST_NEXT_ERROR", req, resp);
          adf.mf.api.amx.hideLoadingIndicator();
        });
      }
    }
    ...
    innerListElement)
  {
    ...
    {
      var amxNodeId = removed[i];
      var domElement = document.getElementById(amxNodeId);
      if (domElement != null && domElement.parentNode != null)
      {
        var parentNode = domElement.parentNode;
        adf.mf.api.amx.removeDomNode(domElement);
        ...
        if (parentNode.classList.contains("amx-listView-group"))
        {
          ...
          if (parentNode.querySelector(".amx-listItem.amx-node") == null)
          {
            // There are no list items, remove the divider group
    ...
    if (collectionChanges["rowsAdded"])
    {
      ...
      while (iter.hasNext())
      {
        ...
        if (child != null && created[key] == true)
        {
          ...
          this._renderStampedItem(amxNode, innerListElement, child, previousListItemAmxNode,
            previousListItemAmxNodeElement);
        }
    ...
    if (dividerValue != null)
    {
      ...
      else
      {
        ...
        if (dividerGroup == null)
        {
          ...
          this._appendToListView(innerListElement, dividerGroup, previousGroupElement);
    ...
    previousListItemAmxNodeElement)
  {
    ...
    if (listItemElement != null)
    {
      ...
      else // No dividers
      {
        ...
        {
          this._appendToListView(innerListElement, listItemElement,
            previousListItemAmxNodeElement);
        }
        else
        {
          ...
          if (firstListElement == null)
          {
            ...
            this._appendToListView(innerListElement, listItemElement, null);
          }
          else
          {
  ...
  listView.prototype._displayDividerCount = function(innerListElement)
  {
    ...
    for (var i = 0, childCount = listViewChildren.length; i < childCount; ++i)
    {
      ...
      if (dividerGroup.classList.contains("amx-listView-group"))
      {
        ...
        {
          if (children[childIndex].classList.contains("amx-listItem") &&
            !children[childIndex].classList.contains("amx-listItem-moreRows") &&
            !children[childIndex].classList.contains("amx-listView-divider"))
          {
            count++;
          }
        }
  ...
  listView.prototype._getCurrentDivider = function(amxNode, dividerAttrEl)
  {
    ...
    if (dividerAttributeValue === undefined)
    {
      throw new adf.mf.PropertyNotFoundException(adf.mf.resource.getInfoString("AMXInfoBundle",
        "amx_listView_LABEL_UNKNOWN_DIVIDER_ATTRIBUTE",amxNode.getAttribute("dividerAttribute")));
    }
  ...
  listView.prototype._handleMoreRowsAction = function(amxNode)
  {
    ...
    var perfOp = adf.mf.internal.perf.startOperation(
      "amx:listView.loadMoreRows",
      "Load more rows for list view with ID " + amxNode.getId());
  ...
  function findListViewAncestor(element)
  {
    if (typeof element !== undefined)
    {
      ...
      {
        if (parentElement.classList.contains("amx-listView"))
        {
