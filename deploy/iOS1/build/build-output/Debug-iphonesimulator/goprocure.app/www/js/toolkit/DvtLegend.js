...
DvtLegend.prototype.Init = function(context, callback, callbackObj) {
  ...
  this._peers = [];
  /**
   * The array of logical objects that corresponds to section collapse buttons.
   * @private
   */
  this._buttonPeers = [];
  /**
...
DvtLegend.prototype.__getKeyboardObjects = function() {
  ...
  if (hideAndShow != 'none' && hideAndShow != 'off')
    return this._buttonPeers.concat(this._peers);
  else
    return this._buttonPeers;
};
...
DvtLegendEventManager.prototype._processActionEvent = function(obj) {
  ...
  if (obj instanceof DvtLegendObjPeer && obj.isDrillable()) {
    var text = obj.getData()['text'];
    this.FireEvent(new DvtDrillEvent(text, text, null), this._legend);
    return true;
  }
...
DvtLegendKeyboardHandler.prototype.processKeyDown = function(event) {
  ...
  var em = this._legend.__getEventManager();
  ...
  if (keyCode == DvtKeyboardEvent.TAB) {
    ...
    if (currentNavigable) {
      if (isButton)
        em.onCollapseButtonClick(event, currentNavigable.getDisplayables()[0]);
      else
        em.processHideShowEvent(currentNavigable);
      DvtEventManager.consumeEvent(event);
    }
  }
 ...
 */
var DvtLegendObjPeer = function(legend, displayables, id, tooltip, drillable) {
  this.Init(legend, displayables, id, tooltip, drillable);
};
 ...
 * @param {boolean} drillable Whether the legend item is drillable.
 */
DvtLegendObjPeer.prototype.Init = function(legend, displayables, item, tooltip, drillable) {
  this._legend = legend;
 ...
 */
DvtLegendObjPeer.associate = function(displayables, legend, item, tooltip, drillable) {
  // Item must have displayables and an id to be interactive.
  if (!displayables || !item)
    return null;

  // Create the logical object.
  var identObj = new DvtLegendObjPeer(legend, displayables, item, tooltip, drillable);
...
DvtLegendObjPeer.prototype.getAriaLabel = function() {
  ...
  if (this._displayables[0] instanceof DvtButton) {
    states = [DvtBundle.getTranslatedString(DvtBundle.UTIL_PREFIX, this.getData()['expanded'] == 'off' ? 'STATE_COLLAPSED' : 'STATE_EXPANDED')];
    return DvtDisplayable.generateAriaLabel(this.getData()['title'], states);
  }

  if (hideAndShow != 'off' && hideAndShow != 'none') {
    states = [DvtBundle.getTranslatedString(DvtBundle.UTIL_PREFIX, bHiddenCategory ? 'STATE_HIDDEN' : 'STATE_VISIBLE')];
    return DvtDisplayable.generateAriaLabel(this.getData()['text'], states);
  }
...
DvtLegendRenderer._renderContents = function(legend, container, availSpace) {
  ...
  if (title) {
    ...
    availSpace.h -= Math.floor(titleDim.h + titleGap); // Bug 17368977: IE9 attributes slightly too much height for the legend title
  }
...
DvtLegendRenderer._createButton = function(context, legend, item, resources, prefix, x, y, tooltip, id, callback, callbackObj) {
  ...
  var peer = DvtLegendObjPeer.associate([button], legend, item, tooltip, false);
...
DvtLegendRenderer._createLegendItem = function(legend, container, item, availSpace, textSpace, rowHeight, i) {
  ...
  var peer = DvtLegendObjPeer.associate(displayables, legend, item, tooltip, DvtLegendRenderer._isItemDrillable(legend, item));
...
DvtLegendRenderer._createMarker = function(legend, x, y, colWidth, rowHeight, markerSize, item) {
  ...
  var markerY = y + rowHeight / 2;
  var markerX = x + colWidth / 2;

  var legendMarker;
  if (pattern && (pattern != 'none')) {
    // Pattern markers must be translated, since the pattern starts at the origin of the shape
    legendMarker = new DvtSimpleMarker(context, shape, legendOptions['skin'], 0, 0, markerSize, markerSize);
    legendMarker.setFill(new DvtPatternFill(pattern, color, '#FFFFFF'));
    legendMarker.setTranslate(markerX, markerY);
  }
  else {
    legendMarker = new DvtSimpleMarker(context, shape, legendOptions['skin'], markerX, markerY, markerSize, markerSize);
