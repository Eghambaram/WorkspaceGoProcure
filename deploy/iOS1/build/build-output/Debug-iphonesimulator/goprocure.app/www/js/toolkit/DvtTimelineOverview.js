...
DvtTimelineOverview.prototype.addMarker = function(node, sz)
{
  ...
  if (this.isVertical())
  {
    marker = DvtMarker.SQUARE;
    var displayable = new DvtMarker(this.getCtx(), marker, null, node.getY(), node.getX(), 2, 2, itemId, scaleX, scaleY);
  }
  else
    displayable = new DvtMarker(this.getCtx(), marker, null, node.getX(), node.getY(), sz, sz, itemId, scaleX, scaleY);
  ...
  if ((this.isVertical() || marker == DvtMarker.SQUARE || marker == DvtMarker.DIAMOND || marker == DvtMarker.TRIANGLE_UP ||
      marker == DvtMarker.TRIANGLE_DOWN || marker == DvtMarker.PLUS) && this._defaultMarkerStyles.pixelHinting != 'false')
  {
  ...
  for (var i = this._maxDurationY; i > 0; i--)
  {
    for (var j = 0; j < durationMarkers.length; j++)
    {
      ...
      if (i == node._durationLevel)
      {
        ...
        if (this.isVertical())
          var duration = new DvtRect(context, this.Width - durationY, x, durationY, x2 - x, durationId);
        else
        {
          if (this.isRTL())
...
DvtTimelineOverview.prototype.calculateY = function(result, shape, cx, cy, hszx, hszy, maxy, hsz, maxHeight)
{
  ...
  for (var i = 0; i < result.arr.length; i++)
  {
    ...
    if (shape == DvtMarker.CIRCLE && prevShape == DvtMarker.CIRCLE && hszx == hszy && prevScaleX == prevScaleY)
...
{
  if (!(drawable instanceof DvtMarker))
  {
  ...
  {
    var width = parseInt(this.getStyle(state, DvtTimelineOverview.BORDER_WIDTH), 10);
    var borderOffset = parseInt(this.getStyle(state, DvtTimelineOverview.BORDER_OFFSET), 10);

    if (borderMarker == null)
    {
      if (markerType == DvtMarker.CIRCLE)
      {
        if (this.isFlashEnvironment())
          borderOffset = 0;
        borderMarker = new DvtMarker(this.getCtx(), markerType, null, this.getX(drawable) - borderOffset, this.getY(drawable) - borderOffset, drawable.getDimensions().w + (borderOffset * 2), drawable.getDimensions().h + (borderOffset * 2), drawable.getId() + '_border', drawable.getScaleX(), drawable.getScaleY());
      }
      else
      {
        if (this.isVertical())
          borderMarker = new DvtMarker(this.getCtx(), markerType, null, this.getY(drawable) - ((width + 1) / 2), this.getX(drawable) - ((width + 1) / 2), drawable.getDimensions().w + (width + 1), drawable.getDimensions().h + (width + 1), drawable.getId() + '_border', drawable.getScaleX(), drawable.getScaleY());
        else
          borderMarker = new DvtMarker(this.getCtx(), markerType, null, this.getX(drawable) - borderOffset, this.getY(drawable) - borderOffset, drawable.getDimensions().w + (borderOffset * 2), drawable.getDimensions().h + (borderOffset * 2), drawable.getId() + '_border', drawable.getScaleX(), drawable.getScaleY());
      }
    ...
    var stroke = new DvtSolidStroke(borderColor, this.getStyle(state, DvtTimelineOverview.BORDER_OPACITY), width);
    borderMarker.setStroke(stroke);

    // Do not antialias marker borders if specified or vertical
    if ((this.isVertical() || markerType == DvtMarker.SQUARE || markerType == DvtMarker.DIAMOND || markerType == DvtMarker.TRIANGLE_UP ||
             markerType == DvtMarker.TRIANGLE_DOWN || markerType == DvtMarker.PLUS) && this._defaultMarkerStyles.pixelHinting != 'false')
    {
    ...
    if (requiresGlowMarker)
    {
      if (glowMarker == null)
      {
        var glowOffset = borderOffset - width;
        if (markerType == DvtMarker.CIRCLE)
        {
          if (this.isFlashEnvironment())
            glowOffset = 0;
          glowMarker = new DvtMarker(this.getCtx(), markerType, null, this.getX(drawable) - glowOffset, this.getY(drawable) - glowOffset, drawable.getDimensions().w + (glowOffset * 2), drawable.getDimensions().h + (glowOffset * 2), drawable.getId() + '_glow', drawable.getScaleX(), drawable.getScaleY());
        }
        else
        {
          if (this.isVertical())
            glowMarker = new DvtMarker(this.getCtx(), markerType, null, this.getY(drawable), this.getX(drawable) - 1, drawable.getDimensions().w + 3, drawable.getDimensions().h + 3, drawable.getId() + '_glow', drawable.getScaleX(), drawable.getScaleY());
          else
            glowMarker = new DvtMarker(this.getCtx(), markerType, null, this.getX(drawable) - glowOffset, this.getY(drawable) - glowOffset, drawable.getDimensions().w + (glowOffset * 2), drawable.getDimensions().h + (glowOffset * 2), drawable.getId() + '_glow', drawable.getScaleX(), drawable.getScaleY());
        }
...
DvtTimelineOverviewNode.prototype.Init = function(overview, props) 
{
  ...
  this._shape = DvtMarker.CIRCLE;
  if (props.shape == 'square')
    this._shape = DvtMarker.SQUARE;
  else if (props.shape == 'plus')
    this._shape = DvtMarker.PLUS;
  else if (props.shape == 'diamond')
    this._shape = DvtMarker.DIAMOND;
  else if (props.shape == 'triangleUp')
    this._shape = DvtMarker.TRIANGLE_UP;
  else if (props.shape == 'triangleDown')
    this._shape = DvtMarker.TRIANGLE_DOWN;
...
DvtTimelineOverviewAutomation.prototype.GetSubIdForDomElement = function(displayable)
{
  ...
  if (displayable instanceof DvtMarker)
  {
    var arr = id.split(':');
    if (arr.length != 4)
      return null;
 ...
 * @param {number} index
 * @return {DvtMarker} the marker matching the criteria.
 * @private
 */
