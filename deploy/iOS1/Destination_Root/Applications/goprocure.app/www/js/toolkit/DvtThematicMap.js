...
DvtThematicMap.prototype._renderLegend = function() {
  if (this._legendData) {
    ...
    var legendMaxWidth = this._getLegendMaxWidthValue(legendData['maxWidth'], this.getWidth());
    var preferredSize = this._legend.getPreferredSize(legendData, legendMaxWidth, this._legendPanel.getMaxContentHeight());
...
DvtMapObjPeer.prototype.Init = function(data, dataLayer, displayable, label, center) {
  ...
  // WAI-ARIA
  if (this.Displayable && !(this.Displayable instanceof DvtBaseComponent)) {
    if (data['destination']) {
...
DvtMapObjPeer.prototype.getAriaLabel = function() {
  if (!(this.Displayable instanceof DvtBaseComponent)) {
    var states = [];
...
DvtMapObjPeer.prototype.isSelected = function() {
  return this.Displayable.isSelected ? this.Displayable.isSelected() : false;
};
...
/**
 * Helper method to hide the hover effect of this data object
 * @protected
 */
DvtMapObjPeer.prototype.HideHoverEffect = function() {
  if (this.Displayable.hideHoverEffect)
...
DvtMapObjPeer.prototype.showKeyboardFocusEffect = function() {
  ...
  if (this.Displayable instanceof DvtBaseComponent) {
    if (!this._keyboardFocusEffect)
      this._keyboardFocusEffect = new DvtKeyboardFocusEffect(this.Displayable.getCtx(), this.Displayable, new DvtRectangle(0, 0, this.Displayable.getWidth(), this.Displayable.getHeight()));
...
DvtThematicMapJsonParser.prototype.ParseDataLayers = function(dataLayers, parentLayer, topLayerName, isAreaDataLayer) {
  ...
  for (var i = 0; i < dataLayers.length; i++) {
    ...
    var dataLayer = new DvtMapDataLayer(this._tmap, parentLayer, dataLayerOptions['id'], this._tmap.__getEventManager());
    ...
    var components = dataLayerOptions['_components'];
    var markers = dataLayerOptions['markers'];
    if (markers && !components) {
    ...
    if (components) {
      for (var j = 0; j < components.length; j++) {
        ...
        var dataObj = this._createComponent(parentLayer, dataLayer, components[j], isAreaDataLayer);
        if (popups)
          dataObj.setShowPopupBehaviors(popups);
        if (dataObj) {
          // Selection support
 ...
 * @private
 */
DvtThematicMapJsonParser.prototype._createComponent = function(layer, dataLayer, data, isParentAreaDataLayer) {
  var center = DvtThematicMapJsonParser.getCenter(dataLayer, data);
  if (!center) // no matching city
    return null;
  ...
  return new DvtMapObjPeer(data, dataLayer, data['displayable'], null, center);
};
