...
DvtChartAxis.prototype.getCoordAt = function(value) {
  return this.Info ? this.axisToPlotArea(this.Info.getCoordAt(value)) : null;
};
...
DvtChartAxis.prototype.getBoundedCoordAt = function(value) {
  return this.Info ? this.axisToPlotArea(this.Info.getBoundedCoordAt(value)) : null;
};
...
DvtChartAxis.prototype.getUnboundedCoordAt = function(value) {
  return this.Info ? this.axisToPlotArea(this.Info.getUnboundedCoordAt(value)) : null;
};


/**
 * Returns the group information for the specified group. Only applies to group axis.
 * @param {number} index The group index.
 * @return {string} The group label.
 */
DvtChartAxis.prototype.getGroup = function(index) {
  if (this.isGroupAxis())
    return this.Info ? this.Info.getGroup(index) : null;
  else
    return null;
};
 ...
 * @return {number} The width of a single group for an axis.  Returns 0 for unsupported axes.
 */
DvtChartAxis.prototype.getGroupWidth = function() {
  return this.Info ? this.Info.getGroupWidth() : 0;
};
...
DvtChartAxis.prototype.getLength = function() {
  return this.Info ? Math.abs(this.Info.getStartCoord() - this.Info.getEndCoord()) : null;
};
...
DvtChartAxis.prototype.getMinCoord = function() {
  return this.Info ? this.axisToPlotArea(Math.min(this.Info.getStartCoord(), this.Info.getEndCoord())) : null;
};
...
DvtChartAxis.prototype.getMaxCoord = function() {
  return this.Info ? this.axisToPlotArea(Math.max(this.Info.getStartCoord(), this.Info.getEndCoord())) : null;
};
// Copyright (c) 2011, 2015, Oracle and/or its affiliates. All rights reserved.
/**
...
DvtChartBar.prototype._setBarCoords = function(baselineCoord, endCoord, x1, x2, bAdjustForGaps) {
  ...
  if (this._dataItemGaps > 0 && bAdjustForGaps && !this.isSelected()) {
    ...
    if (barWidth >= DvtChartBar._MIN_BAR_WIDTH_FOR_GAPS) {
      if (DvtAgent.getDevicePixelRatio() == 1 && barWidth > DvtChartBar._MIN_BAR_WIDTH_FOR_GAPS_PIXEL_HINTING) {
        // If devicePixelRatio is 1, we need to be extremely precise since anti-aliasing must be disabled for correct
        // gaps. Don't do this for FF though, since it does pixel hinting incorrectly.
...
DvtChartOverview.prototype._renderChart = function(options, width, height) {
  ...
  var optionsOverride = {
    ...
    'xAxis': {
      ...
      'axisLine': {'rendered': 'off'}, 'maxSize': 0.5, 'title': null
    },
    'yAxis': {'rendered': 'off'},
    'y2Axis': {'rendered': 'off'},
    'splitDualY': 'off',
 ...
 * @private
 */
DvtChartPolarBar.prototype._setBarCoords = function(baselineCoord, endCoord, x1, x2, bAdjustForGaps) {
  ...
  var ir = Math.abs(endCoord - baselineCoord) >= DvtChartPolarBar._MIN_BAR_LENGTH_FOR_GAPS && (this._axisCoord != baselineCoord) && bAdjustForGaps ?
...
DvtFunnelSlice.prototype.Init = function(chart, seriesIndex, numDrawnSeries, funnelWidth, funnelHeight, startPercent, valuePercent, fillPercent, gap) {
  ...
  this._3dRatio = (options['styleDefaults']['threeDEffect'] == 'on') ? 1 : 0;
  this._gap = gap;
  var cmds = this._getPath();
  this._dataColor = DvtChartStyleUtils.getColor(this._chart, this._seriesIndex, 0);
  this._backgroundColor = options['styleDefaults']['backgroundColor'];
...
/** Color for the funnel's grey background. Could be overridden in styleDefaults.
 * @private */
...
DvtPieChart.prototype.Init = function(chart, availSpace, callback, callbackObj) {
  ...
  var radiusScale = (this._labelPosition == 'inside' || this._labelPosition == 'none') ? DvtPieChart._RADIUS : DvtPieChart._RADIUS_LABELS;
...
DvtPieChart.prototype._onEnd = function() {
  ...
  // Bug 21559871 - need to re render at the end of the display animation
  this._setAnimationParams();
};
...
DvtPieSlice.prototype.getTooltip = function() {
  if (this._seriesIndex == null) // other slice
    return DvtChartTooltipUtils.getOtherSliceDatatip(this._chart, this._value, DvtChartDefaults.isPostAltaSkin(this._chart));

  return DvtChartTooltipUtils.getDatatip(this._chart, this._seriesIndex, 0, DvtChartDefaults.isPostAltaSkin(this._chart));
};
...
DvtChart.prototype.GetComponentDescription = function() 
{
  ...
  if (options['title'] && options['title']['text'])
    return DvtBundle.getTranslation(options, 'labelAndValue', DvtBundle.UTIL_PREFIX, 'COLON_SEP_LIST', [compName, this.Options['title']['text']]);
  else
    return compName;
};
...
DvtChart.prototype.render = function(options, width, height) 
{
  ...
  var oldChart = (this.Options && animationOnDataChange != 'none') ? new DvtDCChart(this) : null;
  ...
  if (!DvtChartTypeUtils.isOverview(this) && !DvtChartTypeUtils.isSpark(this)) {
    var hideAndShow = DvtChartEventUtils.getHideAndShowBehavior(this);
    if (this.legend && (hideAndShow != 'none' && hideAndShow != 'off'))
      context.setKeyboardFocusArray([this, this.legend]);
    else
      context.setKeyboardFocusArray([this]);
  }
...
DvtChart.prototype.getObject = function(seriesIndex, groupIndex) {
  for (var i = 0; i < this.Peers.length; i++) {
    if (this.Peers[i].getSeriesIndex() == seriesIndex && this.Peers[i].getGroupIndex() == groupIndex)
      return this.Peers[i];
  }
  return null;
};
...
DvtChart.prototype._setScrollbarViewport = function(bounds) {
  if (this.overview && bounds.xMin != null && bounds.xMax != null)
    this.overview.setViewportRange(bounds.xMin, bounds.xMax);
  if (this.xScrollbar && bounds.xMin != null && bounds.xMax != null)
    this.xScrollbar.setViewportRange(bounds.xMin, bounds.xMax);
  if (this.yScrollbar && bounds.yMin != null && bounds.yMax != null)
...
DvtChartAutomation.prototype.GetSubIdForDomElement = function(displayable) {
  ...
  if (displayable.isDescendantOf(this._xAxis)) {
    ...
    logicalObj = this._chart.__getEventManager().GetLogicalObject(displayable);
...
DvtChartAutomation.prototype.getDataItem = function(seriesIndex, itemIndex) {
  ...
  var data = {
    ...
    'open': dataItem ? dataItem['open'] : null,
    'close': dataItem ? dataItem['close'] : null,
 ...
 * @override
 */
DvtChartEventManager.prototype.OnDblClick = function(event) {
  DvtChartEventManager.superclass.OnDblClick.call(this, event);
 ...
 * @override
 */
DvtChartEventManager.prototype.OnComponentTouchDblClick = function(event) {
  DvtChartEventManager.superclass.OnComponentTouchDblClick.call(this, event);
 ...
 * @private
 */
DvtChartEventManager.prototype._processActionEvent = function(obj) {
 ...
 * @private
 */
DvtChartEventManager.prototype._processDrillEvent = function(obj) {
...
DvtChartObjPeer.prototype.Init = function(chart, displayables, seriesIndex, groupIndex, dataPos) {
  ...
  // Bug 20801233. Need to evaluate these up front because the series and group are used for animation
  if (seriesIndex >= 0)
...
DvtChartObjPeer.prototype.getDatatip = function(target) {
  return DvtChartTooltipUtils.getDatatip(this._chart, this._seriesIndex, this._groupIndex, DvtChartDefaults.isPostAltaSkin(this._chart));
};
...
DvtChartObjPeer.prototype.getAriaLabel = function() {
  var states = [];
  if (this.isSelectable())
    states.push(DvtBundle.getTranslation(this.getChart().getOptions(), this.isSelected() ? 'stateSelected' : 'stateUnselected', DvtBundle.UTIL_PREFIX, this.isSelected() ? 'STATE_SELECTED' : 'STATE_UNSELECTED'));
 ...
 * @param {string} tooltip The reference object tooltip
 ...
 * @implements {DvtLogicalObject}
 */
var DvtRefObjPeer = function(displayables, refObj, tooltip, index, axisType) {
  this.Init(displayables, refObj, tooltip, index, axisType);
};

DvtObj.createSubclass(DvtRefObjPeer, DvtSimpleObjPeer, 'DvtRefObjPeer');
 ...
 * @param {string} axisType The axis the reference object is on
 */
DvtRefObjPeer.prototype.Init = function(displayables, refObj, tooltip, index, axisType) {
  DvtRefObjPeer.superclass.Init.call(this, tooltip, null, null, null);
...
DvtChartDefaults.VERSION_1 = {
  ...
  'zoomAndScroll': 'off', 'initialZooming': 'none', 'dragMode': 'user',
  ...
  'styleDefaults': {
    ...
    'sliceLabelStyle': new DvtCSSStyle('font-family: tahoma, sans-serif; font-size: 11px;'),
...
DvtDataCursorHandler.prototype._showDataCursor = function(plotRect, x, y, bSuppressEvent) {
  ...
  // If "dataCursor" attr is "auto", don't show the data cursor if no tooltip text is present. Otherwise, always show the cursor.
  var tooltipText = DvtChartTooltipUtils.getDatatip(this._chart, closestMatch.sidx, closestMatch.gidx, DvtChartDefaults.isPostAltaSkin(this._chart));
  if ((tooltipText == null || tooltipText == '') && this._chart.getOptions()['dataCursor'] != 'on') {
...
DvtDataCursorHandler.prototype._getClosestMatch = function(x, y) {
  ...
  var useAllInGroup = DvtChartTypeUtils.isLineArea(this._chart);
...
DvtDCFunnelSlice.prototype.animateUpdate = function(handler, oldDC) {
  ...
  // TODO hzhang this only works for slices without target values. Slices with target values are drawn within
  // DvtFunnelSlice, so wait until the animation code is collapsed to do that work.
  nodePlayable.getAnimator().addProp(DvtAnimator.TYPE_FILL, this._shape, this._shape.getFill, this._shape.setFill, endFill);

  // TODO hzhang this line of code makes no sense, since we never draw an indicator for funnel
  if (this._indicator) {
    nodePlayable.setOnEnd(this._onEndAnimation, this);
  }
...
DvtChartAxisUtils.getTimeAxisType = function(chart) {
  ...
  if (timeAxisType && DvtChartTypeUtils.isBLAC(chart) && !DvtChartTypeUtils.isPolar(chart))
    return timeAxisType;
  if (DvtChartTypeUtils.isStock(chart))
    return 'regular';
  return 'disabled';
...
DvtChartAxisUtils.isMinorTickRendered = function(chart, type) {
  var axisOptions = chart.getOptions()[type + 'Axis'];
  return axisOptions['rendered'] != 'off' && axisOptions['minorTick']['rendered'] == 'on';
};
...
DvtChartAxisUtils.getGroupWidthRatios = function(chart) {
  ...
  // Store the average z-value. This is useful because when we call groupAxis.getGroupWidth(), it returns the average
  // group width. Thus, we can convert z-value to pixels using (zValue / averageGroupZ * groupAxis.getGroupWidth()).
...
DvtChartDataUtils.processDataObject = function(chart) {
  ...
  for (var i = 0; i < seriesCount; i++) {
    ...
    if (DvtChartTypeUtils.isFunnel(chart) || isStock) // turn off legend series display for funnel and stock
    ...
    // Bug 19402471 - SANITIZE CHART OPTIONS OBJECT
...
DvtChartDataUtils.isXInViewport = function(chart, seriesIndex, groupIndex) {
  ...
  if (isXInViewport == null) {
    ...
    if (isNaN(xVal) || xVal == null)
      ...
      isXInViewport = !(minMax['min'] != null && xVal < minMax['min']) && !(minMax['max'] != null && xVal > minMax['max']);
    }
    // Cache the value
 ...
 * @return {boolean}
 */
DvtChartDataUtils.isDataItemInViewport = function(chart, seriesIndex, groupIndex) {
 ...
 */
DvtChartDataUtils.getMinMaxValue = function(chart, type, excludeBubbleRadii) {
  // Use the cached value if it has been computed before
  var cacheKey = type + 'MinMax';
  var minMax = chart.getFromCache(cacheKey);
  if (!excludeBubbleRadii && minMax)
    return minMax;
  ...
  var limitToViewport = isYValue && DvtChartTypeUtils.isBLAC(chart);
  ...
  for (var seriesIndex = 0; seriesIndex < seriesCount; seriesIndex++) {
    ...
    for (var groupIndex = 0; groupIndex < seriesData.length; groupIndex++) {
      ...
      if (isYValue) {
        ...
        if (DvtChartAxisUtils.hasTimeAxis(chart) && !DvtChartAxisUtils.isMixedFrequency(chart) && data['x'] == null) {
      ...
      if (!isRange && !isNaN(value)) {
        var radius = 0;
        if (DvtChartTypeUtils.isBubble(chart) && !excludeBubbleRadii && (data['markerSize'] > 0)) {
          if (isYValue)
...
DvtChartDataUtils.getStackCategories = function(chart, type, bIncludeHiddenSeries) {
  ...
  for (var s = 0; s < DvtChartDataUtils.getSeriesCount(chart); s++) {
    if ((type && DvtChartStyleUtils.getSeriesType(chart, s) != type && DvtChartStyleUtils.getSeriesType(chart, s) != 'candlestick') || (!DvtChartStyleUtils.isSeriesRendered(chart, s) && !bIncludeHiddenSeries))
      continue;
    ...
    if (DvtChartDataUtils.isAssignedToY2(chart, s)) {
      ...
      if (yCategories.indexOf(category) == -1)
...
DvtChartDataUtils.getBarInfo = function(chart, seriesIndex, groupIndex, availSpace) {
  ...
  var stackWidth = DvtChartStyleUtils.getBarStackWidth(chart, category, groupIndex, bY2Series);

  // Bug 20703222: Mac FF pixel snaps greedily, so there must be 2 or more pixels of gaps.
...
DvtChartEventUtils.setVisibility = function(chart, category, visibility) {
  ...
  // Bug 19195451 - HIDE & SHOW FOR REFERENCE OBJECTS
...
DvtChartEventUtils.getAxisStartEndGroup = function(axis, min, max) {
  if (axis.isGroupAxis() && min != null && max != null) {
    ...
    if (endIdx >= startIdx) {
      var startGroup = axis.getGroup(startIdx);
      var endGroup = axis.getGroup(endIdx);
...
/**
 * Returns reference object found by text (title) in chart.
 * @param {object} chart
 * @param {string} title Reference object title (text)
 * @return {object}
 */
DvtChartRefObjUtils.getRefObj = function(chart, title) {
  var objects = DvtChartRefObjUtils.getObjects(chart);
  for (var i = 0; i < objects.length; i++) {
    if (objects[i]['text'] == title) {
      return objects[i];
    }
  }
};
...
/**
 * Computes the offsets of the bar stack categories relative to the group coordinate and stores is in a map.
 * @param {DvtChart} chart
 * @param {Number} groupIndex
...
DvtChartStyleUtils.getBarCategoryOffsetMap = function(chart, groupIndex) {
  ...
  for (var category in yOffsetMap)
    yOffsetMap[category] -= yTotalWidth / 2;
  for (var category in y2OffsetMap)
    y2OffsetMap[category] -= y2TotalWidth / 2;
 ...
 * Possible values are: center, inLeft, inRight, inTop, inBottom, left, right, top, bottom
 */
 ...
 * Use this method instead of calling chart.xAxis.getGroupWidth() directly for better performance.
 * @param {DvtChart} chart
 * @return {number}
 */
DvtChartStyleUtils.getGroupWidth = function(chart) {
  ...
  if (width == null) {
    width = chart.xAxis.getGroupWidth();
...
DvtChartTooltipUtils._addGroupDatatip = function(datatip, chart, seriesIndex, groupIndex, isTabular) {
  ...
  if (DvtChartAxisUtils.hasTimeAxis(chart)) {
    ...
    if (groupLabel == null)
      groupLabel = chart.xAxis.__getInfo().formatLabel(value);
  }
  else
...
DvtChartTooltipUtils._addDatatipRow = function(datatip, chart, type, defaultLabel, value, isTabular, index) {
  ...
  if (tooltipLabel == null) {
    if (defaultLabel == null) // non-innermost hierarchical group labels
      ...
      // Bug 20861183: Use date instead of group for time axis chart tooltips. Only doing this for JET right now until
...
DvtChartMarkerUtils._getAxisValueRange = function(chart, type) {
  var axisOptions = chart.getOptions()[type + 'Axis'];
  var zeroBaseline = DvtChartAxisUtils.getBaselineScaling(chart, type) == 'zero';
...
DvtPieLabelUtils.layoutLabelsAndFeelers = function(pie) {
  ...
  if (labelPosition == 'none')
    ...
    DvtPieLabelUtils._layoutInsideLabels(pie, true);
    DvtPieLabelUtils._layoutOutsideLabelsAndFeelers(pie, true);
  }
 ...
 * @private
 */
DvtPieLabelUtils._layoutOutsideLabelsAndFeelers = function(pie, isHybrid) {
  var leftLabels = [];
  var rightLabels = [];
  ...
  // -----------------------------------------------------------
  var alabels = DvtPieLabelUtils._generateInitialLayout(pie, isHybrid);
...
DvtPieLabelUtils.createPieCenterLabel = function(pieChart) {
  ...
  if (pieCenterLabel) {
    ...
    DvtTextUtils.fitText(centerLabel, availSpace.w, availSpace.h, pieChart);
...
DvtPieLabelUtils.generateSliceLabelString = function(slice, labelType) {
  ...
  var decDigits = percentage < 0.01 ? 4 : percentage < 0.10 ? 3 : percentage < 1 ? 2 : 0;
  ...
  if (percentConverter && percentConverter['getAsString']) {
    ...
    spercent = DvtChartTooltipUtils.formatValue(pieChart.getCtx(), {}, percentage, null, null, percentage == 100 ? 1 : null) + '%';
  }
 ...
 * @private
 */
DvtPieLabelUtils._generateInitialLayout = function(pie, isHybrid) {
  ...
  for (var i = 0; i < slices.length; i++) {
    var slice = slices[i];
    if (isHybrid && slice.getSliceLabel() != null)
      continue;  // use inside label
    ...
    var labelPt = DvtPieRenderUtils.reflectAngleOverYAxis(ma, pie.getCenter().x, pie.getCenter().y, pie.getRadiusX() * dist, pie.getRadiusY() * dist);
    ...
    var tmDimPt = DvtPieLabelUtils._getTextDimension(pie, s_label, maxLabelWidth, DvtPieLabelUtils._MAX_LINES_PER_LABEL);// set up for word wrap
    var midArea = 15;
...
DvtChartRenderer.renderEmptyText = function(chart, container, availSpace) {
  ...
  DvtTextUtils.renderEmptyText(container, emptyTextStr,
      ...
      chart.__getEventManager(), options['title']['style']);
};
...
DvtChartAxisRenderer._renderCartesian = function(chart, container, availSpace) {
  ...
  if (bAligned && yInfo && y2Info) {
    ...
    // Bug 19849117 - y2 tick label is missing sometimes
...
DvtChartAxisRenderer._alignYAxes = function(yInfo, y2Info) {
  ...
  y2Options['step'] = y2Axis.getMajorIncrement();
  y2Options['minorStep'] = y2Axis.getMinorIncrement();
};
...
DvtChartLegendRenderer._createLegendItem = function(chart, seriesIndex) {
  ...
  // Action, popup, and drill support
...
DvtPlotAreaRenderer._renderForegroundObjects = function(chart, container, availSpace) {
  // Fix for 19475329: Axis lines are generally rendered in the back of the foreground, but we render them after the
...
DvtPlotAreaRenderer._getMarkerInfo = function(chart, seriesIndex, groupIndex, availSpace) {
  ...
  if (!bMarkerDisplayed) {
    if (DvtChartAxisUtils.isMixedFrequency(chart)) {
      // For mixed frequency, points are connected across nulls.
...
DvtPlotAreaRenderer._renderBars = function(chart, container, availSpace) {
  ...
  for (var groupIndex = 0; groupIndex < DvtChartDataUtils.getGroupCount(chart); groupIndex++) {
    ...
    for (var seriesIndex = 0; seriesIndex < DvtChartDataUtils.getSeriesCount(chart); seriesIndex++) {
      if (!DvtChartDataUtils.isDataItemInViewport(chart, seriesIndex, groupIndex) || DvtChartStyleUtils.getSeriesType(chart, seriesIndex) != 'bar')
        continue;
...
DvtPlotAreaRenderer._renderStock = function(chart, container, availSpace) {
  ...
  // Iterate through the data
  for (var groupIndex = 0; groupIndex < DvtChartDataUtils.getGroupCount(chart); groupIndex++) {
    if (!DvtChartDataUtils.isDataItemInViewport(chart, 0, groupIndex))
      continue;
...
DvtPlotAreaRenderer._filterScatterBubble = function(chart, bSortBySize, availSpace) {
  ...
  for (var seriesIndex = 0; seriesIndex < seriesCount; seriesIndex++) {
    ...
    for (var groupIndex = 0; groupIndex < groupCount; groupIndex++) {
      markerInfos.push(DvtPlotAreaRenderer._getMarkerInfo(chart, seriesIndex, groupIndex, availSpace));
    }
  }
...
DvtPlotAreaRenderer._getCoordsForSeries = function(chart, seriesIndex, availSpace, type) {
  ...
  for (var groupIndex = 0; groupIndex < DvtChartDataUtils.getGroupCount(chart); groupIndex++) {
    ...
    if (yValue == null || isNaN(yValue) || !DvtChartDataUtils.isDataItemInViewport(chart, seriesIndex, groupIndex)) {
...
DvtFunnelRenderer.render = function(chart, container, availSpace) {
  ...
  if (chart.getOptions()['orientation'] == 'horizontal')
    ...
    var rotationMatrix = new DvtMatrix();
    if (DvtAgent.isRightToLeft(chart.getCtx())) {
      rotationMatrix.translate(- availSpace.y - availSpace.h / 2, - availSpace.x - availSpace.w / 2);
      rotationMatrix.rotate(-Math.PI / 2);
      rotationMatrix.translate(availSpace.y + availSpace.w / 2, availSpace.x + availSpace.h / 2);
    }
    else {
...
DvtRefObjRenderer._renderObjectsForAxis = function(chart, container, plotAreaBounds, location, axis, objects) {
  ...
  for (var i = 0; i < objects.length; i++) {
    ...
    var refObjPeer = new DvtRefObjPeer([shape], refObj, tooltip, i, axisType);
...
DvtRefObjRenderer._createReferenceArea = function(refObj, chart, plotAreaBounds, axis) {
  ...
  if (refObj['items'] != null && axis == chart.yAxis) { // REF AREA WITH MULTIPLE VALUES
    ...
    if (lowVal == null || lowVal == -Infinity)
      lowVal = axis.getGlobalMin();
    if (highVal == null || highVal == Infinity)
      highVal = axis.getGlobalMax();
...
DvtSparkChart.prototype.Init = function(context, callback, callbackObj) {
  ...
  // Create the event handler and add event listeners
  this._eventHandler = new DvtEventManager(context, this.__dispatchEvent, this);
