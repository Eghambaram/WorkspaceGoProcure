 ...
 * @private
 */
DvtTimeComponentAxis._VALID_SCALES = ['hours', 'days', 'weeks', 'months', 'quarters', 'years'];
  ...
  if (this._series)
  {
    ...
    for (var i = 0; i < seriesCount; i++)
    {
      ...
      if (!this._isVertical)
      {
        ...
        var backY = this._startY + (i * (this._seriesSize + this.getTimeAxisSize())) + (this._seriesSize - DvtTimeline.DEFAULT_HOTSPOT_HEIGHT) / 2;
        var forwardY = backY;
      ...
      else
      {
        if (this.isRTL())
          ...
          key = i;
        backX = this._startX + (key * (this._seriesSize + this.getTimeAxisSize())) + (this._seriesSize - DvtTimeline.DEFAULT_HOTSPOT_WIDTH) / 2;
        forwardX = backX;
      ...
      if (!this._isVertical)
      {
        ...
        var bottomX = topX;
        var topY = this._startY + (i * (this._seriesSize + this.getTimeAxisSize())) + DvtTimeline.DEFAULT_HOTSPOT_PADDING;
        var bottomY = this._startY + ((i + 1) * this._seriesSize) + (i * this.getTimeAxisSize()) - DvtTimeline.DEFAULT_HOTSPOT_HEIGHT - DvtTimeline.DEFAULT_HOTSPOT_PADDING;
      ...
      else
      {
        if (this.isRTL())
          ...
          key = i;
        topX = this._startX + (key * (this._seriesSize + this.getTimeAxisSize())) + DvtTimeline.DEFAULT_HOTSPOT_PADDING;
        bottomX = this._startX + ((key + 1) * this._seriesSize) + (key * this.getTimeAxisSize()) - DvtTimeline.DEFAULT_HOTSPOT_WIDTH - DvtTimeline.DEFAULT_HOTSPOT_PADDING;
...
DvtTimeline.prototype.renderAxis = function()
{
  ...
  var axisStart = seriesCount == 1 ? (this._canvasSize - axisSize) : (this._canvasSize / seriesCount - (axisSize / 2));
    ...
    for (var i = 0; i < seriesCount; i++)
    {
      ...
      if (series._isEmpty)
      {
        ...
        if (seriesEmptyText != null)
        {
          ...
          var posMatrix = new DvtMatrix(1, 0, 0, 1, (this._canvasLength - dim.w) / 2 + this._startX, i * (this._seriesSize + this.getTimeAxisSize()) + ((this._seriesSize - dim.h) / 2) + this._startY);
...
DvtTimeline.prototype.updateSeries = function()
{
  ...
  {
    var seriesCount = this._series.length;
    this._seriesSize = (this._canvasSize - this.getTimeAxisSize()) / seriesCount;
    for (var i = 0; i < seriesCount; i++)
    {
      var series = this._series[i];
      ...
      if (this._isVertical)
      {
        ...
        if (this.isRTL() && this._series.length == 1)
        {
          cp.addRect(this.getTimeAxisSize(), 0, this._seriesSize, this._contentLength);
          var posMatrix = new DvtMatrix(1, 0, 0, 1, this.getTimeAxisSize(), 0);
        }
        else
        {
          cp.addRect(key * (this._seriesSize + this.getTimeAxisSize()), 0, this._seriesSize, this._contentLength);
          posMatrix = new DvtMatrix(1, 0, 0, 1, key * (this._seriesSize + this.getTimeAxisSize()), 0);
        }
        var width = this._seriesSize;
        var height = this._contentLength;
      }
      ...
      {
        cp.addRect(0, i * (this._seriesSize + this.getTimeAxisSize()), this._contentLength, this._seriesSize);
        posMatrix = new DvtMatrix(1, 0, 0, 1, 0, i * (this._seriesSize + this.getTimeAxisSize()));
        width = this._contentLength;
        height = this._seriesSize;
      }
      series.setClipPath(cp);
...
DvtTimeline.prototype.panBy = function(deltaX, deltaY)
{
  if (this._isVertical)
  {
    ...
    if (this._dragPanSeries)
    {
      ...
      {
        var minTranslateX = this.getTimeAxisSize() + (2 * this._dragPanSeries.Width) - this._dragPanSeries._maxOverflowValue;
        var maxTranslateX = this._dragPanSeries.Width + this.getTimeAxisSize();
      }
      else if (this.isRTL() && !this._dragPanSeries._isInverted)
      {
        minTranslateX = this._dragPanSeries.Width - this._dragPanSeries._maxOverflowValue + this.getTimeAxisSize();
        maxTranslateX = this.getTimeAxisSize();
      }
      else
      {
        minTranslateX = 0;
  ...
  else
  {
    ...
    if (this._dragPanSeries)
    {
      ...
      {
        var minTranslateY = this.getTimeAxisSize() + (2 * this._dragPanSeries.Height) - this._dragPanSeries._maxOverflowValue;
        var maxTranslateY = this._dragPanSeries.Height + this.getTimeAxisSize();
      }
      else
      {
        var minTranslateY = 0;
...
{
  var itemStart = item.getLoc() - 20;
  var itemEnd = itemStart + (this._isVertical ? item.getHeight() : item.getWidth());
  if (this.isRTL() && !this._isVertical)
    this._startPos = this._backgroundWidth - this._contentLength - this._startPos;
...
DvtTimelineSeries.prototype.Init = function(context, callback, callbackObj)
{
  ...
  if (DvtTimeUtils.supportsTouch())
    DvtToolkitUtils.setAttrNullNS(this._elem, 'id', this._id);
  this._blocks = [];
...
DvtTimelineSeries.prototype.renderReferenceObjects = function()
{
  if (this._referenceObjects)
  {
    ...
    for (var i = 0; i < maxRefObjects; i++)
    {
      ...
      {
        if (this.isRTL())
          var pos = this._length - DvtTimeUtils.getDatePosition(this._start, this._end, refObject, this._length);
        else
          pos = DvtTimeUtils.getDatePosition(this._start, this._end, refObject, this._length);
        if (this._isVertical)
...
DvtTimelineSeries.prototype.updateReferenceObjects = function()
{
  for (var i = 0; i < this._renderedReferenceObjects.length; i++)
  {
    ...
    if (this.isRTL())
      var pos = this._length - DvtTimeUtils.getDatePosition(this._start, this._end, ref.date, this._length);
    else
      pos = DvtTimeUtils.getDatePosition(this._start, this._end, ref.date, this._length);
    if (this._isVertical)
    {
      ref.setX1(0);
      ref.setY1(pos);
