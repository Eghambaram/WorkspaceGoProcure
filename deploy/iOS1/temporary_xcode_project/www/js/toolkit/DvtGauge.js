...
DvtGauge.prototype.render = function(options, width, height) 
{
  ...
  if (this._editingOverlay && this.Options['readOnly'] === false) {
  ...
  if (!this._bStaticRendering && !this.Options['readOnly']) {
    ...
    if (eventManager)
      eventManager.associate(this._editingOverlay, this.GetLogicalObject(), true);
  }
 ...
 * @protected
 */
DvtGauge.prototype.GetLogicalObject = function() {
  var tooltip = DvtGaugeRenderer.getTooltipString(this);
...
DvtGaugeEventManager.prototype.PreEventBubble = function(event) {
  ...
  if (!this.isEditing || this.IsShowingTooltipWhileEditing())
...
DvtGaugeRenderer.renderEmptyText = function(gauge, container, availSpace) {
  ...
  DvtTextUtils.renderEmptyText(container, emptyTextStr,
      ...
      gauge.__getEventManager(), options['metricLabel']['style']);
};
...
DvtGaugeRenderer._formatLabelValue = function(value, gauge, converter, scaling, autoPrecision, isPercent) {
  ...
  if (isPercent) {
    value = DvtGaugeRenderer.getFillPercentage(options, options['min'], options['max'], value);
...
DvtGaugeRenderer.getTooltipString = function(gauge) {
  ...
  if (threshold && threshold['shortDesc'] != null)
    ...
    return DvtGaugeRenderer.getFormattedMetricLabel(options['value'], gauge);
  }
};
 ...
 * @param {boolean} bMouseTransparent true if the label should not recieve mouse events.
 */
DvtGaugeRenderer.renderLabel = function(gauge, container, bounds, color, bMouseTransparent) {
  var options = gauge.getOptions();
  ...
  if (options['metricLabel']['rendered'] == 'on') {
    ...
    if (DvtTextUtils.fitText(label, labelWidth, labelHeight, container)) {
      if (bMouseTransparent)
        label.setMouseEnabled(false);
      else if (label.isTruncated()) // Show tooltip for truncated text
  ...
  if (DvtGaugeDataUtils.hasData(gauge)) {
    ...
    if (options['metricLabel']['rendered'] == 'on') {
      ...
      if (!DvtAgent.isHighContrast() && options['metricLabel']['style'].getStyle('color') != null) {
        DvtGaugeRenderer.renderLabel(gauge, container, DvtLedGaugeRenderer._getLabelBounds(gauge, container, bounds), null, true);
      }
      else {
        ...
        DvtGaugeRenderer.renderLabel(gauge, container, DvtLedGaugeRenderer._getLabelBounds(gauge, container, bounds), labelColor, true);
      }
    }
  }
...
DvtLedGaugeRenderer._renderShape = function(gauge, container, bounds) {
  ...
  if (type == 'rectangle') {
  ...
  else if (type == 'star' || type == 'triangle' || type == 'arrow') {
    if (type == 'star')
    ...
    if (isSkyros && type == 'triangle' || isSkyros && type == 'arrow') {
  ...
  // Tooltip Support
  var eventManager = gauge.__getEventManager();
  var tooltip = DvtGaugeRenderer.getTooltipString(gauge);
  if (tooltip && eventManager)
    eventManager.associate(shape, new DvtSimpleObjPeer(null, tooltip, color));

  // rotate the shape if needed
  var rotation = isSkyros ? options['rotation'] + 90 : options['rotation'];
  if (rotation != 0 && (type == 'arrow' || type == 'triangle'))
...
DvtStatusMeterGaugeDefaults.VERSION_1 = {
  'color': '#313842',
  'indicatorSize': 1,
  'metricLabel': {'style': new DvtCSSStyle('font-family: tahoma, sans-serif; color: #333333;')},
...
DvtStatusMeterGaugeRenderer.render = function(gauge, container, width, height) {
  if (DvtGaugeDataUtils.hasData(gauge)) {
    ...
    if (options['orientation'] == 'horizontal' || options['orientation'] == 'vertical') {
      ...
      var labelBounds = new DvtRectangle(bounds.x + bounds.w / 2 - maxDiameter * .3, bounds.y + bounds.h / 2 - maxDiameter * .25, maxDiameter * .6, maxDiameter * .5);
      DvtGaugeRenderer.renderLabel(gauge, container, labelBounds, null, true);
...
DvtStatusMeterGaugeRenderer._renderCircularShape = function(gauge, container, bounds) {
  ...
  var thresholds = options['thresholds'];
  var maxDiameter = Math.min(bounds.w, bounds.h);
  var innerRadius = maxDiameter * .35;
  ...
  if (referenceObjects) {
    for (var i = 0; i < referenceObjects.length; i++) {
      ...
      var angle = Math.PI * 1.5 + DvtGaugeRenderer.getFillPercentage(options, options['min'], options['max'], value) * 2 * Math.PI;
  ...
  // Tooltip
  var tooltip = DvtGaugeRenderer.getTooltipString(gauge);
  if (tooltip || gauge.getOptions()['readOnly'] === false)
...
DvtStatusMeterGaugeRenderer._renderShape = function(gauge, container, bounds) {
  ...
  var plotX1 = bounds.x;
  var plotX2 = bounds.x + bounds.w;
  var plotY1 = bounds.y;
  var plotY2 = bounds.y + bounds.h;
  var indicatorX1, indicatorX2, indicatorY1, indicatorY2 = 0;
  if (isVert) {
  ...
  if (thresholds && options['plotArea']['rendered'] != 'off' && options['thresholdDisplay'] == 'all') {
    for (var i = thresholds.length - 1; i >= 0; i--) {
      ...
      if (i == thresholds.length - 1) {
        ...
        if (isVert) {
          cp.addRect(0, axisInfo.getUnboundedCoordAt(options['max']), plotX2 - plotX1 + 2, (options['max'] - thresholds[thresholds.length - 2 - currentThresholdIndex]['max']) * 1 / (Math.abs(options['min'] - options['max'])) * bounds.h, 0, 0);
        }
        else {
          if (isRTL) {
            ...
            cp.addRect(0, 0, (options['max'] - thresholdMax) * 1 / (Math.abs(options['min'] - options['max'])) * bounds.w, plotY2 - plotY1 + 2, 0, 0);
          }
          else
            cp.addRect(0, 0, (thresholds[currentThresholdIndex]['max'] - options['min']) * 1 / (Math.abs(options['min'] - options['max'])) * bounds.w, plotY2 - plotY1 + 2, 0, 0);
        }
      }
  ...
  var shape = DvtStatusMeterGaugeRenderer._createShape(gauge, gauge.getCtx(), indicatorX1, indicatorX2, indicatorY1, indicatorY2);
  ...
  if (referenceObjects) {
    for (var i = 0; i < referenceObjects.length; i++) {
      ...
      // Shadowing effect
      // Fix for bug 20136668: Shadow causes the reference line to disappear in IE11
...
DvtStatusMeterGaugeRenderer._createShape = function(gauge, context, x1, x2, y1, y2) {
  return new DvtStatusMeterGaugeIndicator(gauge, context, x1, x2, y1, y2);
};
...
DvtStatusMeterGaugeIndicator.prototype.Init = function(gauge, context, x1, x2, y1, y2) {
  ...
  this._gauge = gauge;
  this._isVert = gauge.getOptions()['orientation'] == 'vertical';
...
DvtDialGaugeRenderer._renderTickLabels = function(gauge, container, bounds) {
  ...
  if (options['background']['radius'] && options['background']['majorTickCount']) {
    ...
    for (var x = 0; x < majorTickCount; x++)
    {
      ...
      // Truncate if needed, null is returned if the label doesn't fit
      if (DvtTextUtils.fitText(label, labelBounds.w + .5, labelBounds.h + .5, container)) {
        // Show tooltip for truncated text
        if (label.isTruncated())
 ...
 * @override
 */
DvtRatingGauge.prototype.GetLogicalObject = function() {
...
DvtRatingGaugePeer.prototype.getDatatip = function(target, x, y) {
  ...
  var thresholdIndex;
  if (this._gauge.__getEventManager().isEditing)
...
DvtRatingGaugeRenderer.render = function(gauge, container, width, height) {
  if (DvtGaugeDataUtils.hasData(gauge)) {
    ...
    // Tooltip Support
    var tooltip = (threshold && threshold['shortDesc']) ? threshold['shortDesc'] : options['shortDesc'];
    if (tooltip)
      gauge.__getEventManager().associate(overlayRect, new DvtRatingGaugePeer(gauge));
  }
  else // Render the empty text
    DvtGaugeRenderer.renderEmptyText(gauge, container, new DvtRectangle(0, 0, width, height));
};
...
DvtRatingGaugeEventManager.prototype.OnMouseOver = function(event) {
  ...
  if (this._gauge.getOptions()['readOnly'] === false && !this._bValueChanged) {
    var coords = this.GetRelativePosition(event.pageX, event.pageY);
