...
DvtNBox.prototype.__getDragTransferable = function(node) {
  ...
  for (var i = 0; i < selection.length; i++) {
    ...
    if (item instanceof DvtNBoxNode) {
      rowKeys.push(item.getData()[DvtNBoxConstants.ID]);
    }
    else if (item instanceof DvtNBoxCategoryNode) {
      var nodeIndices = item.getData()['nodeIndices'];
      for (var j = 0; j < nodeIndices.length; j++) {
        rowKeys.push(DvtNBoxDataUtils.getNode(this, nodeIndices[j])[DvtNBoxConstants.ID]);
      }
    }
  }
...
//
// $Header: dsstools/modules/dvt-shared-js/src/META-INF/bi/sharedJS/toolkit/adfinternal/nBox/DvtNBoxBundle.js /bibeans_root/12 2014/10/21 08:55:20 breliu Exp $
//
// DvtNBoxBundle.js
//
...
DvtNBoxDefaults.VERSION_1 = {
  ...
  'styleDefaults': {
    ...
    'cellDefaults': {'style': new DvtCSSStyle('background-color:#dddddd'),
    ...
    '__drawerDefaults': {'background': '#dddddd',
      'borderColor': '#c4ced7',
      'borderRadius': 1,
...
//
// $Header: dsstools/modules/dvt-shared-js/src/META-INF/bi/sharedJS/toolkit/nBox/DvtNBoxNodeOverflow.js /st_jdevadf_pt-jet.v110/2 2015/05/08 11:28:06 amdai Exp $
//
// DvtNBoxNodeOverflow.js
//
...
//
//    DESCRIPTION
//     wrapper for the overflow image
//
//    MODIFIED  (MM/DD/YY)
//    nbalatsk   07/15/14 - Created
//
...
DvtNBoxNodeOverflow.prototype.render = function(container, width, height) {
  ...
  this._button = new DvtButton(ctx, upState, overState, downState, null, null, this.HandleClick, this);
...
DvtNBoxCategoryNode.prototype.Init = function(nbox, data) {
  DvtNBoxCategoryNode.superclass.Init.call(this, nbox.getCtx(), null,
                                           isNaN(data[DvtNBoxConstants.CELL]) ? data[DvtNBoxConstants.ID] : data[DvtNBoxConstants.CELL] + ':' + data[DvtNBoxConstants.ID]);// TODO for JRAMANAT: Passing non container params weird
  this._nbox = nbox;
  this._data = data;
  this._nbox.registerObject(this);
  var selectionMode = this._nbox.getOptions()[DvtNBoxConstants.SELECTION_MODE];
...
//
// $Header: dsstools/modules/dvt-shared-js/src/META-INF/bi/sharedJS/toolkit/nBox/DvtNBoxKeyboardHandler.js /st_jdevadf_pt-jet.v110/4 2015/05/08 11:28:06 amdai Exp $
//
...
DvtNBoxKeyboardHandler.getNextNavigableCategoryNode = function(curr, event, navigableItems) {
  ...
  if (navigableItems[0] instanceof DvtNBoxCategoryNode) {
    navigableItems.sort(function(a, b) {return a.getData()['nodeIndices'].length < b.getData()['nodeIndices'].length;});
  }
...
DvtNBoxRenderer._renderLegend = function(nbox, container, availSpace) {
  ...
  if (legendData && legendData['sections']) {
    ...
    panelDrawer.addPanel(legendContainer, legendEnaImg, legendOvrImg, legendDwnImg, DvtBundle.getTranslatedString(DvtBundle.UTIL_PREFIX, 'LEGEND'), 'legend');
    if (rtl) {
          ...
          if (!DvtNBoxDataUtils.isCellMinimized(nbox, cellIndex)) {
            ...
            if (!skipNodes) {
              ...
              if (maxNodes < 0 || gridPos[cellIndex] < maxNodes) {
                ...
                var scrolling = cellContainer instanceof DvtSimpleScrollableContainer ? true : false;
      ...
      if (groups[i] && !DvtNBoxDataUtils.isCellMinimized(nbox, i)) {
        ...
        for (var group in positions) {
          ...
          nodeContainer.setTranslate(cell['__childArea'].x + cell['__childArea'].w / 2 + scale * (positions[group].x - center.x),
                                     (childContainer instanceof DvtSimpleScrollableContainer ? 0 : cell['__childArea'].y) + cell['__childArea'].h / 2 + scale * (positions[group].y - center.y));
...
DvtNBoxNodeRenderer._renderNodeIcon = function(nbox, node, nodeContainer, nodeLayout) {
  ...
  if (icon) {
    ...
    var iconWidth = icon[DvtNBoxConstants.WIDTH] ? icon[DvtNBoxConstants.WIDTH] : (nodeLayout['iconSectionWidth'] - 2 * padding);
    var iconHeight = icon[DvtNBoxConstants.HEIGHT] ? icon[DvtNBoxConstants.HEIGHT] : (nodeLayout['nodeHeight'] - 2 * padding);
    var iconX = nodeLayout[rtl ? 'labelSectionWidth' : 'indicatorSectionWidth'] + nodeLayout['iconSectionWidth'] / 2;
...
DvtNBoxNodeRenderer.animateDelete = function(animationHandler, oldNode) {
  ...
  var id = oldNode.getData()[DvtNBoxConstants.ID];
...
DvtNBoxDrawerRenderer._renderBody = function(nbox, data, drawerContainer) {
  ...
  // Render the nodes
  data['__childArea'] = new DvtRectangle(gridGap, drawerHeaderHeight + gridGap, drawerBounds.w - 2 * gridGap, drawerBounds.h - drawerHeaderHeight - 2 * gridGap);
...
//
// $Header: dsstools/modules/dvt-shared-js/src/META-INF/bi/sharedJS/toolkit/nBox/utils/DvtNBoxDataUtils.js /st_jdevadf_pt-jet.v110/5 2015/05/08 11:28:06 amdai Exp $
//
// DvtNBoxDataUtils.js
//
// Copyright (c) 2013, 2015, Oracle and/or its affiliates. All rights reserved.
