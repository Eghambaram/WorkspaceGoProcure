...
DvtText.prototype.Init = function(dvtContext, textString, x, y, id)
{
  ...
  //specified by DvtTextArea subclass
...
{
  //Bug Fix 20126842 - nls:hv:bidi:search panel does not display the search results data
...
DvtText.prototype.impl_setCSSStyle = function(style)
{
  ...
  if (style) {
    ...
    //For multi line text, text-align is handled in DvtTextArea
...
DvtText.prototype.impl_setEditMode = function(bEditMode, editStartCallback, editEndCallback) {
  ...
  //FIX BUG 16901294: in edit mode, make this text a shape container and add a rect behind the text
  ...
  if (bEditMode) {
    ...
    //FIX BUG 18324259 - ie11: html5 hv search field has very small font and vertical scroll bar visible
...
DvtText.prototype.impl_SetEditing = function(bEditing) {
  ...
  else                                       // change to editMode
  {
    ...
    this.getElem().removeChild(textElem);
    //FIX BUG 16901294: add the HTML text area to the SVG parent div so that it can be overlaid on top of SVG content
    if (this._editElem) {
...
DvtText.prototype.impl__handleHtmlTextAreaKeyboardEvent = function(event) {
  ...
  //FIX BUG 16901294: the HTML text area will be flagged so that DvtHtmlKeyboardListenerUtils ignores its native events,
...
DvtText.prototype.impl_focusEditText = function() {
  //FIX BUG 16901294: actively editing when text field receives focus
...
DvtText.prototype.impl__createHtmlTextArea = function(obj, textElem, editText) {
  ...
  //FIX BUG 16901294: absolutely position the HTML text area over the DvtText
  ...
  //FIX BUG 16901294: mark the HTML text area so that DvtHtmlKeyboardListenerUtils will ignore its native events,
...
//FIX BUG 16901294: when editing, getElem() will not return the text element, so provide a function
 ...
 * @param {String} id  Optional ID for the shape (see {@link  DvtDisplayable#setId}).
 */
var DvtTextArea = function(dvtContext, x, y, id) {
 ...
 */
DvtTextArea.prototype.Init = function(dvtContext, x, y, id) {
  ...
  DvtTextArea.superclass.Init.call(this, dvtContext, '', x, y, id);
...
DvtTextArea.prototype._wrapText = function() {
  ...
  tmptf.setCSSStyle(this.getCSSStyle());
  ...
  for (var lineIndex = 0; lineIndex < numLines; lineIndex++) {
    truncateNonWrappedString = false;
    ...
    if (dim.w > maxLineWidth) {
      ...
      if (remainder) {
        ...
        //Bug 16949028 - AF:OUTPUTTEXT INSIDE DIGRAMNODES CANNOT DISPLAY CORRECTLY
        ...
        if (preWrappedString == remainder) {
          ...
          var wordOffsets = DvtTextArea._breakWords(preWrappedString);
          if (wordOffsets.length > 1) {
            remainder = preWrappedString.substring(wordOffsets[1].index);
    ...
    //Bug 16949028 - AF:OUTPUTTEXT INSIDE DIGRAMNODES CANNOT DISPLAY CORRECTLY
    maxOut = bMaxHeight || ((this._maxLines != -1) && (lineIndex == this._maxLines - 1));

    if (maxOut || truncateNonWrappedString)
    {
      if (bMaxHeight || (this._maxLines < numLines) || truncateNonWrappedString) {
        ...
        // enable truncation
        tmptf.setMaxWidth(maxLineWidth, this._minChars);
        this._bTruncated = true;
      }
    }
  ...
  //TODO: call API
  //peformance: need in DvtTextArea
  this._maxh = max_hh;
  this._maxw = max_ww;
 ...
 * @override
 */
DvtTextArea.prototype.copyShape = function()
{
  var copy = new DvtTextArea(this.getCtx(), this.getX(), this.getY());
 ...
 * @param {array} dimList Optional dimensions of multilines
 */
DvtTextArea.prototype.impl_setText = function(multilines, dimList) {
  var singleLine;
  ...
  // keep the old dominant-baseline
  var dbl = this.impl__getCurrentDominantBaseline();
  ...
  //FIX BUG 16856961: save multiline dimensions for vertical alignment
  this._multilineDims = null;
  ...
  if (obj.getNumChildren() === 0 && singleLine) {
    ...
    // Otherwise create it
      ...
      this.impl__createTextNode(singleLine);
    }
  }
  else {
    var oldx = parseFloat(DvtToolkitUtils.getAttrNullNS(elem, 'x'));
    var oldy = parseFloat(DvtToolkitUtils.getAttrNullNS(elem, 'y'));
    if (isNaN(oldx)) {
      // FIX Bug 16588881 - js chart: pie labels may overflow when the pie is small
      // Problem with multiline labels resolved by following check
      oldx = 0;
    }
    if (isNaN(oldy)) {
      oldy = 0;
    ...
    if (singleLine) {
      this.impl__addLine(obj, singleLine, oldx, oldy);
    }
    else {
      var x = 0;
      //line position needs to start at specified value
      var y = oldy;
      var dim;
      var maxDim = new DvtRectangle(oldx, oldy, obj._getMaxTextWidth(), obj._getMaxTextHeight());

      //FIX BUG 16856961: save the multiline dimensions for vertical alignment
      this._multilineDims = [];
      for (var i = 0; i < multilines.length; i++) {
        dim = (dimList && dimList[i]) ? dimList[i] : maxDim;
        this._multilineDims.push(dim);

        x = oldx;
        if (this._notGraph)
          x += this.impl__getHorzOffset(dim.w);

        this.impl__addTextLine(obj, multilines[i], x, y, dbl);
  ...
  //FIX BUG 16856961: refresh vertical alignment
  this.impl_RefreshVertAlign();
};
 ...
 * @param {string} text the text string
 */
DvtTextArea.prototype.impl_getText = function() {
  var obj = this.getObj();
  var nkids = obj.getNumChildren();

  if (nkids == 0) {
    return DvtTextArea.superclass.impl_getText.call(this);
  }
  else {
    var child;
    var text = '';
 ...
 * @return {number} the amount of vertical space (called leading) between lines
 */
DvtTextArea.prototype.impl_getLeading = function() {
 ...
 * @param {number} leading amount of vertical space, in pixels, between lines
 */
DvtTextArea.prototype.impl_setLeading = function(leading) {
  this._leading = leading;
};

// add a text line
DvtTextArea.prototype.impl__addLine = function(obj, newLineText, x, dy) {
  var newDvtText = this.impl__createTSpan(newLineText);
  DvtToolkitUtils.setAttrNullNS(newDvtText, 'x', x);
  DvtToolkitUtils.setAttrNullNS(newDvtText, 'dy', dy + this.impl_getLeading());
...
// add a text line
DvtTextArea.prototype.impl__addTextLine = function(obj, newLineText, x, y, dbl) {
  var newDvtText = new DvtText(obj.getCtx(), newLineText, x, y, null);
  newDvtText.setParent(obj);
  ...
  var attr = DvtToolkitUtils.getAttrNullNS(textNode, 'dominant-baseline');
  if (attr) {
    DvtToolkitUtils.setAttrNullNS(newDvtText.getElem(), 'dominant-baseline', attr);
  }
  else if (dbl) {
    DvtToolkitUtils.setAttrNullNS(newDvtText.getElem(), 'dominant-baseline', dbl);
  }

  attr = DvtToolkitUtils.getAttrNullNS(textNode, 'text-anchor');
  if (attr)
    DvtToolkitUtils.setAttrNullNS(newDvtText.getElem(), 'text-anchor', attr);

  attr = DvtToolkitUtils.getAttrNullNS(textNode, 'font-family');
  if (attr)
    DvtToolkitUtils.setAttrNullNS(newDvtText.getElem(), 'font-family', attr);

  attr = DvtToolkitUtils.getAttrNullNS(textNode, 'font-size');
  if (attr)
    DvtToolkitUtils.setAttrNullNS(newDvtText.getElem(), 'font-size', attr);

  attr = DvtToolkitUtils.getAttrNullNS(textNode, 'fill');
  if (attr)
    DvtToolkitUtils.setAttrNullNS(newDvtText.getElem(), 'fill', attr);

  attr = DvtToolkitUtils.getAttrNullNS(textNode, 'cursor');
  if (attr)
    DvtToolkitUtils.setAttrNullNS(newDvtText.getElem(), 'cursor', attr);

  // Note: This code doesn't get called in anything except FF v24, and it's unclear whether it's useful or not.  Once FF24 is removed,
  // we should just get rid of it.  Fake usage to help search:
  // DvtAgent.isPlatformGecko() && DvtAgent.getVersion() < 25;
  if (DvtAgent.isRightToLeft(obj.getCtx())) {
    attr = DvtToolkitUtils.getAttrNullNS(textNode, 'unicode-bidi');
    if (attr)
      DvtToolkitUtils.setAttrNullNS(newDvtText.getElem(), 'unicode-bidi', attr);
  }

  //FIX BUG 16856961: remove vertical alignment transform on child text so it
  //doesn't interfere with vert align transform on this text area
  if (DvtAgent.isPlatformIE()) {
    DvtToolkitUtils.setAttrNullNS(newDvtText.getElem(), 'transform', null);
    DvtToolkitUtils.removeAttrNullNS(newDvtText.getElem(), 'transform');
  }

  this.addChild(newDvtText);
};


/**
  * Returns "left", "center", and "right".
  * "start" and "end" are converted to "left" and "right" based on locale
  * @return {string} "left", "center" or "right"
  */
DvtTextArea.prototype.impl__getTextAlign = function() {
  var style = this.getCSSStyle();
  var textAlign = style.getStyle('text-align');

  var obj = this.getObj();
  var isRTL = DvtAgent.isRightToLeft(obj.getCtx());
 ...
 * @override
 */
DvtTextArea.prototype.setCursor = function(cursorType) {
  DvtTextArea.superclass.setCursor.call(this, cursorType);

  var elem = this.getElem();
  if (elem != null) {
    for (var i = 0; i < elem.childElementCount; i++) {
      var child = elem.childNodes[i];
      DvtToolkitUtils.setAttrNullNS(child, 'cursor', cursorType);
    }
  }
};


DvtTextArea.prototype.impl__getHorzOffset = function(ww) {
  var offset = 0;
  var obj = this.getObj();
  var textAlign = this.impl__getTextAlign();

  if (textAlign == 'right') {
    offset = obj._getMaxTextWidth();
  }
  else if (textAlign == 'center') {
    offset = obj._getMaxTextWidth() / 2;
  }
  return offset;
};
...
/**
 * @protected
 * Refresh vertical alignment when text changes.
 */
DvtTextArea.prototype.impl_RefreshVertAlign = function() {
  //Bug 13879007 - NODE RENDERED INCORRECTLY IN IE9
  if (DvtAgent.isPlatformIE()) {
    this.impl_SetBaseline(this._baseline);
  }
  else {
    this.impl__adjustMultiBaselines();
  }
};
 ...
 * @override
 */
DvtTextArea.prototype.impl_SetBaseline = function(baseline) {
  //if there are multiple lines, calculate an additional pixel baseline offset
  this._baselineOffset = null;
  if (this._multilineDims) {
    var accumHeight = 0;
    //the first line is already shifted correctly for a single line, so only
    //need to take additional lines into account to determine extra shift
    for (var j = 1; j < this._multilineDims.length; j++) {
      accumHeight += this._multilineDims[j].h;
    }
    switch (this.getObj().getVertAlignment()) {
      case DvtText.V_ALIGN_MIDDLE:
        this._baselineOffset = -.5 * accumHeight;
        break;
      case DvtText.V_ALIGN_BOTTOM:
      case DvtText.V_ALIGN_BASELINE:
        this._baselineOffset = -accumHeight;
        break;
    }
  }
  DvtTextArea.superclass.impl_SetBaseline.call(this, baseline);
};
 ...
 * Set the dominant baseline for vertical alignment.
 */
DvtTextArea.prototype.impl_SetDominantBaselineAttr = function(baseline) {
  DvtTextArea.superclass.impl_SetDominantBaselineAttr.call(this, baseline);

  //FIX BUG 16856961: adjust baselines for vertical alignment
  this.impl__adjustMultiBaselines();
};
...
/**
 * @private
 * Get the current value of dominant-baseline.
 */
DvtTextArea.prototype.impl__getCurrentDominantBaseline = function() {
  var elem = this.getElem();
  var dbl;
  if (elem && elem.nodeName == 'g') {
    var childNodes = elem.childNodes;
    if (childNodes && childNodes.length > 0 && childNodes[0].nodeName == 'text') {
      dbl = DvtToolkitUtils.getAttrNullNS(childNodes[0], 'dominant-baseline');
    }
  }
  return dbl;
};
...
/**
 * @private
 * Adjust baselines for vertical alignment of multi-line text.
 */
DvtTextArea.prototype.impl__adjustMultiBaselines = function() {
  var elem = this.getElem();
  if (elem && this._multilineDims) {
    var valign = this.getObj().getVertAlignment();
    var baselineShiftAttr = 'baseline-shift';
    var dominantBaselineAttr = 'dominant-baseline';
    var dominantBaselineValue = this.impl__getCurrentDominantBaseline();
    var px = 'px';
    var numChildren = elem.childElementCount;
    var accumHeight = 0;
    ...
    for (var i = 0; i < numChildren; i++) {
      ...
      //copy the dominant-baseline from first line to other lines
      if (dominantBaselineValue) {
        DvtToolkitUtils.setAttrNullNS(child, dominantBaselineAttr, dominantBaselineValue);
      }
      else {
        DvtToolkitUtils.removeAttrNullNS(child, dominantBaselineAttr);
      }

      //shift the baseline
      switch (valign) {
        case DvtText.V_ALIGN_TOP:
          DvtToolkitUtils.removeAttrNullNS(child, baselineShiftAttr);
          break;
        case DvtText.V_ALIGN_MIDDLE:
          DvtToolkitUtils.setAttrNullNS(child, baselineShiftAttr, (.5 * accumHeight) + px);
          break;
        case DvtText.V_ALIGN_BOTTOM:
        case DvtText.V_ALIGN_BASELINE:
          DvtToolkitUtils.setAttrNullNS(child, baselineShiftAttr, accumHeight + px);
          break;
      }
    }
  }
...
/**
 * Sets the DvtCSSStyle of this object.
 * Note: If style contains text-alignment, it will remove text-anchor of its children text
 * and set the specified text-alignment on the text-anchor
 */
DvtTextArea.prototype.impl_setCSSStyle = function(style) {
  if (this._notGraph) {
    var elem = this.getOuterElem();

    var numChildren = elem.childElementCount;
    if (numChildren > 1) {
      var val = this.impl__getTextAlign();
      var alignAttr = 'text-anchor';

      for (var i = 0; i < numChildren; i++) {
        var child = elem.childNodes[i];
        DvtToolkitUtils.removeAttrNullNS(child, alignAttr);
      }

      // alignLeft and alignRight are actually browser independent alignLeft and alignRight
      if (val) {
        if (val == 'left') {
          this.impl_alignLeft();
        }
        ...
        DvtToolkitUtils.removeAttrNullNS(elem, alignAttr);
      }
    }
  }

  DvtTextArea.superclass.impl_setCSSStyle.call(this, style);
};
// Copyright (c) 2008, 2015, Oracle and/or its affiliates. All rights reserved.
/*-------------------------------------------------------------------------*/
  ...
  if (afStyles && afStyles.length > 0) {
    ...
    for (var i = 0; i < afStyles.length; i++) {
      ...
      if (styles.name == 'defaultFont') {
        ...
        DvtAfStyleUtils._setDefaultFontColor(font.getStyle(DvtCSSStyle.FONT_COLOR));
      }
...
DvtAfStyleUtils.getDefaultPanelCardStyles = function() {
  ...
  defStyle.setStyle(DvtCSSStyle.FONT_FAMILY, DvtAfStyleUtils.getDefaultFontFamily());
  defStyle.setStyle(DvtCSSStyle.FONT_WEIGHT, 'bold');
  defStyle.setStyle(DvtCSSStyle.FONT_SIZE, '20px');
  defStyle.setStyle(DvtCSSStyle.COLOR, '#5A6A7A');
...
DvtAfComponent.prototype.adjustClientArea = function(cssStyle) {
    ...
    if (num > -1) {
      ...
      if (aWidth !== Number.MAX_VALUE && sw != null) {
        if (this.isContentBox()) {
          ...
          //BUG FIX 14313127: make sure number is not negative
  ...
  //BUG FIX 17184456: Enable container clipping if the content width/height is greater than available width/height
...
DvtAfComponent.prototype.createTextStyles = function(style, defaultStyle) {
  ...
  if (! style.getStyle(k)) {
    ...
    //BUG FIX #10275298 and #10274543: if no text-align is specified, CSS treats it
...
DvtAfComponent.prototype.createDisplayObj = function(afContext) {
  ...
  //BUG FIX 13971862: save the parent node so we can call back to it for popups
...
/**
 * @protected
 * Handle click for the AfComponents that fire ActionEvent
 */
DvtAfComponent.prototype.HandleClickAction = function(event) {
};
...
DvtAfComponent.prototype.SaveContextCallback = function() {
  if ((this.getShowPopupBehaviors && this.getShowPopupBehaviors()) || this.getClientBehaviors() ||
    ...
    this._contextCallbackObj = this.getAfContext().getContextCallbackObj();
    this._contextCallback = this.getAfContext().getContextCallback();
  }
...
//BUG FIX 13971862: return the bounds of the object to use for aligning popups
...
DvtAfComponentUtils._calcVContentSize = function(afContext, afRoot, arDim, isLink) {
  ...
  for (var i = 0; i < cnt; i++) {
    ...
    if (! child.renderMe || child.renderMe()) {
      ...
      //BUG FIX 13942023: remove component when either width or height is 0
...
DvtAfComponentFactory.parseAndLayout = function(afContext, templateTree, parent) {
  ...
  //BUG FIX 14456277: Save the parent node for aligning popups
...
DvtAfButtonLink.prototype.RegisterListeners = function() {
  var shape = this.getContentShape();
  if (shape) {
    shape.addEvtListener(DvtTouchEvent.TOUCHSTART, this.getClickListenerFunction(), false, this);
    if (!DvtAgent.isTouchDevice()) {
      shape.addEvtListener(DvtMouseEvent.CLICK, this.getClickListenerFunction(), false, this);
      shape.addEvtListener(DvtMouseEvent.MOUSEOVER, this.HandleRollOver, false, this);
      shape.addEvtListener(DvtMouseEvent.MOUSEOUT, this.HandleRollOut, false, this);

      shape.addEvtListener(DvtMouseEvent.MOUSEDOWN, this.HandleMouseDown, false, this);
      shape.addEvtListener(DvtMouseEvent.MOUSEUP, this.HandleMouseUp, false, this);
    }
...
/**
 * Adds callback to the displayable object represented by DvtButton
 */
DvtAfButtonLink.prototype.AddCallback = function() {
  var callback = DvtToolkitUtils.getLinkCallback(this.getTargetFrame(), this.getDestination());
  if (callback) {
    var shape = this.getContentShape();
    shape.setCallback(callback);
  }
};
...
// event handlers
DvtAfButtonLink.prototype.HandleRollOver = function(event) {
};

DvtAfButtonLink.prototype.HandleRollOut = function(event) {
};

DvtAfButtonLink.prototype.HandleMouseDown = function(event) {
};

DvtAfButtonLink.prototype.HandleMouseUp = function(event) {
};
...
DvtAfButtonLink.prototype.CreateButton = function(button) {
  ...
  if (! button) {
    return new DvtButton(this.getDvtContext(), null, null, null, null,
                         this._getUniqueId());
  }

  // create all mouse states
  if (this.isDisabled()) {
    //    var up = this.CreateUpState();
    var dis = this.CreateDisabledState();
    //    button.setUpState(up);
    button.setDisabledState(dis);
    button.setEnabled(false);
  }
  else {
    var over = this.CreateOverState();
    var down = this.CreateDownState();
    var up = this.CreateUpState();

    button.setOverState(over);
    button.setDownState(down);
    button.setUpState(up);
 ...
 *
 * @return display obj to use for button "up" state
 */
DvtAfButtonLink.prototype.CreateUpState = function() {
  return this.CreateButtonState(DvtButton.STATE_ENABLED);
};


/**
 * Create the "down" state of the button.
 *
 * @return display obj to use for button "down" state
 */
DvtAfButtonLink.prototype.CreateDownState = function() {
  return this.CreateButtonState(DvtButton.STATE_DOWN);
};


/**
 * Create the "over" state of the button.
 *
 * @return display obj to use for button "over" state
 */
DvtAfButtonLink.prototype.CreateOverState = function() {
  return this.CreateButtonState(DvtButton.STATE_OVER);
};


/**
 * Create the "disabled" state of the button.
 *
 * @return display obj to use for button "disabled" state
 */
DvtAfButtonLink.prototype.CreateDisabledState = function() {
  return this.CreateButtonState(DvtButton.STATE_DISABLED);
};
...
/**
 * Render the "up" state of the button.
 *
 * @return display obj to use for button "up" state
 */
DvtAfButtonLink.prototype.RenderUpState = function() {
  var ss = this._getStateStyle(DvtButton.STATE_ENABLED);
  return this._renderButtonState(ss, DvtButton.STATE_ENABLED);
};


/**
 * Render the "down" state of the button.
 *
 * @return display obj to use for button "down" state
 */
DvtAfButtonLink.prototype.RenderDownState = function() {
  var ss = this._getStateStyle(DvtButton.STATE_DOWN);
  return ss ? this._renderButtonState(ss, DvtButton.STATE_DOWN) : null;
};


/**
 * Render the "over" state of the button.
 *
 * @return display obj to use for button "over" state
 */
DvtAfButtonLink.prototype.RenderOverState = function() {
  var ss = this._getStateStyle(DvtButton.STATE_OVER);
  return ss ? this._renderButtonState(ss, DvtButton.STATE_OVER) : null;
};


/**
 * Render the "disabled" state of the button.
 *
 * @return display obj to use for button "disabled" state
 */
DvtAfButtonLink.prototype.RenderDisabledState = function() {
  var ss = this._getStateStyle(DvtButton.STATE_DISABLED);
  return ss ? this._renderButtonState(ss, DvtButton.STATE_DISABLED) : null;
};
 ...
 *
 * @param style CSSStyle to use for the state
 * @param buttonState button state: up, down, or over
 *
 * @return display obj to use for button state
 * @private
 */
DvtAfButtonLink.prototype._renderButtonState = function(style, buttonState) {
...
DvtAfButtonLink.prototype.RenderButton = function() {
  ...
  var dim;
  if (this.isDisabled()) {
    dim = this.RenderDisabledState();
  }
  else {
    this.RenderOverState();
    this.RenderDownState();
    dim = this.RenderUpState();
  }
...
DvtAfButtonLink.prototype.handleKeyboardEvent = function(event) {
  ...
  if (keyCode == DvtKeyboardEvent.ENTER) {
    ...
    if ((this.getTargetFrame() || this.getDestination())) {
      //links
 ...
 * @override
 */
DvtAfLink.prototype.HandleMouseUp = function(event) {
  var shape = this.getContentShape();
  if (shape)
 ...
 * @override
 */
DvtAfLink.prototype.HandleRollOver = function(event) {
  var shape = this.getContentShape();
  if (shape)
...
DvtAfCommandMenuItem.prototype.HandleClickAction = function(event) {
  var menu = this.getParent();
  if (menu instanceof DvtAfMenu) {
    menu.handleMenuItemSelected(event);
  }
};
 ...
 * @override
 */
DvtAfMenu.prototype.HandleClickAction = function(event) {
  ...
  if (this.getNumChildren() > 0) {
    ...
    //display the dropdown containing the commandMenuItems
    this.displayDropdown();
  }

  /*
  var callback = this.getContextCallback();
  var callbackObj = this.getContextCallbackObj();
  if (callback) {
    callback.call(callbackObj, null);
  }
  */
};
...
DvtAfMenu.prototype.handleDropdownRollOut = function(event) {
  this.hideDropdown();
};
 ...
 */
DvtAfMenu.prototype.handleMenuItemSelected = function(event) {
  //consume event
  // if the dropdown is visible, just hide it.
  // Note: need to keep command menu items around to handle the event
  this.hideDropdown();
};
...
DvtAfMenu.prototype.displayDropdown = function() {
  ...
  if (stage) {
    stage.addEvtListener(DvtMouseEvent.MOUSEDOWN, this.HandleStageFocusChange, true, this);
  }
...
DvtAfMenu.prototype.populateDropdown = function(afContext, dropdown) {
  ...
  for (var i = 0; i < cmiShapes.length; i++) {
    ...
    if (itemButton) {
      ...
      //      itemButton.addEvtListener(DvtMouseEvent.CLICK, this.handleMenuItemSelected, false, this);
    }
  }
...
DvtAfMenu.prototype.RemoveStageFocusChangeListeners = function() {
  var stage = this.getDvtContext().getStage();
  if (stage) {
    stage.removeEvtListener(DvtMouseEvent.MOUSEDOWN, this.HandleStageFocusChange, true, this);
  }
};
...
DvtAfMarker._markerTypeMap = {
  ...
  's' : DvtMarker.SQUARE,
  'd' : DvtMarker.DIAMOND,
  'tu' : DvtMarker.TRIANGLE_UP,
  'td' : DvtMarker.TRIANGLE_DOWN,
  'p' : DvtMarker.PLUS,
  'h' : DvtMarker.HUMAN,
  'rr' : DvtMarker.ROUNDED_RECTANGLE
};
...
DvtAfMarker.prototype.getBorderWidth = function() {
  return this.getProperty(DvtAfMarker.ATTR_BORDER_WIDTH);
};
...
DvtAfMarker.prototype.createDisplayObj = function(afContext) {
  ...
  //BUG FIX 13971862: save the parent node so we can call back to it for popups
  //BUG FIX 14456277: Save the parent node for markers, because they do not call super.createDisplayObj
  this._parentNode = afContext.getParentNode();
  ...
  // create marker
  var marker = new DvtMarker(dvtContext, type, null, x, y, ww, hh, id + '_m', scaleX, scaleY);
...
DvtAfMarker.prototype.render = function(afContext) {
  ...
  var msw = marker.getScaledWidth();
  var msh = marker.getScaledHeight();
...
DvtAfSpacer.prototype.createContentShape = function(dvtContext) {
  this._contentPane = new DvtRect(dvtContext, 0, 0, 1, 1, this.getId());
  this._contentPane.setFill(null);
  return this._contentPane;
};
...
DvtAfPanelCard.prototype._insertChild = function(oldsdi, newsdi) {
  ...
  //Fix Bug 20407864 - panel card refresh issue if more than 5 cards present in hv
...
DvtAfPanelCard.prototype.TransitionBitmaps = function(
  ...
  //BUG FIX #10080242: include title bar in transition
