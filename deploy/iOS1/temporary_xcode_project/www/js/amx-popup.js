...
// TODO finish the migration from "amx.*" to "adf.mf.api.amx.*" and "adf.mf.internal.amx.*"
  ...
  amx.processClosePopupBehavior = function(amxNode, closePopupBehaviorTagInstance)
  {
      ...
      (popupIdToClose == null || popupIdToClose == ""))
    {
      ...
      // TODO this needs to be promoted to a formal warning; do not use amx.log
      amx.log.debug(
  ...
  popup.prototype._showPopup = function(amxNode, completelyNewPopup)
  {
   ...
   for (var i=0, size = descendants.length; i<size; ++i)
   {
    ...
    if (startPositions.hasAnchor === false && popupAnchor)
    {
  ...
  function _calcTabForShowPopup(pageElems)
  {
    ...
    for (var k=0; k < noOfPageElems; k++)
    {
      ...
      var oldTabIndex = pageElems[k].tabIndex;
      var prePopupLevel = pageElems[k].getAttribute("data-popupLevel");

      if (prePopupLevel == null)
      {
        pageElems[k].setAttribute("data-popupLevel", "0");
        pageElems[k].setAttribute("data-oldTabIndex", oldTabIndex);
      }
      else
      {
        var updatePopupLevel = Number(prePopupLevel)-1;
        pageElems[k].setAttribute("data-popupLevel", updatePopupLevel);
      }

      // set tabindex to "-1"
      pageElems[k].setAttribute("tabindex", "-1");
    }
  }
  ...
  function _calcTabForClosePopup(pageElems)
  {
    ...
    {
      // use data-popupLevel to cumulate the level of element.
      var prePopupLevel = pageElems[j].getAttribute("data-popupLevel")
      if (prePopupLevel != null)
      {
        ...
        if(prePopupLevel == "0")
        {
          var oldTabIndex = pageElems[j].getAttribute("data-oldTabIndex");
          pageElems[j].setAttribute("tabindex", oldTabIndex);
          pageElems[j].removeAttribute("data-popupLevel");
          pageElems[j].removeAttribute("data-oldTabIndex");
        }
        else
        {
          var updatePopupLevel = Number(prePopupLevel)+1;
          pageElems[j].setAttribute("data-popupLevel", updatePopupLevel);
        }
      }
    }
  }
