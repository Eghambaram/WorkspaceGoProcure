...
(function ()
{
   ...
   * @param {adf.mf.api.amx.AmxNode} amxNode masonryLayout amx node
   */
  masonryLayout.prototype.getDescendentChangeAction = function (domNode, amxNode)
  {
  ...
  masonryLayout.prototype.refresh = function (amxNode, attributeChanges, descendentChanges)
  {
    try
    {
      ...
      var order = this._getInitialOrder(amxNode);
      var newNodeIndex = [];
      ...
      for (var i = 0; i < amxNodes.length; i++)
      {
        ...
        if (oldNodes[childId])
        {
          subElement = oldNodes[childId];
          if (subElement)
            delete oldNodes[childId];
          moveNode(_masonryLayout, subElement, _masonryLayout[position]);
          position++;
          continue;
        }
        if (subElement)
        {
          newNodes.push(subElement);
      ...
      for (var id in oldNodes)
      {
        ...
        var _amxNode = adf.mf.api.amx.AmxNode.getAmxNodeForElement(node);
        _amxNode.setAttributeResolvedValue("ignoreDrag", true);
        oldNodes2.push(node);
      }
  ...
  masonryLayout.prototype.initMLC = function (amxNode, _masonryLayout)
  {
    ...
    callbackInfo.layoutOnEndFunc = function ()
    {
      ...
      var resizeAmxNode = masonryLayoutAMXN.getAttribute("resizeAmxNode");
      if (resizeAmxNode != null)
      {
        // Tile has just been resized
        resizeAmxNode.setAttributeResolvedValue("ignoreDrag", false);
        masonryLayout._renderDescendants(resizeAmxNode);
        masonryLayoutAMXN.setAttributeResolvedValue("resizeAmxNode", null);
      }
  ...
  masonryLayout.prototype._createMouseDownCallback = function()
  {
    ...
    return function (pageX, pageY, event)
    {
      ...
      {
        if (amxNode.getAttribute("ignoreDrag"))
        {
          return;
        }
  ...
  masonryLayout.prototype._createMouseMoveCallback = function()
  {
    ...
    return function (pageX, pageY, event)
    {
      ...
      if (index > 0)
      {
        ...
        {
          _masonryLayout._mlCommon.setup();
        });
      }
    };
  };
   ...
   */
  masonryLayout.prototype._getInitialOrder = function(amxNode)
  {
    ...
    if (typeof order === "string")
    {
      ...
      for (var i = 0; i < orderArray.length; i++)
        result.push(parseInt(orderArray[i]));
    }
  ...
  masonryLayoutItem.prototype.render = function (amxNode)
  {
    try
    {
      ...
      if (oldTile) {
        ...
        var mlCommon = _masonryLayout._mlCommon;
        amxNode.setAttributeResolvedValue("ignoreDrag", true);
        id = id.replace(/:/g, "\\:");
        var masonryLayoutAMXN = adf.mf.api.amx.AmxNode.getAmxNodeForElement(_masonryLayout);
        masonryLayoutAMXN.setAttributeResolvedValue("resizeAmxNode", amxNode);
  ...
  masonryLayoutItem.prototype.refresh = function (amxNode, attributeChanges, descendentChanges)
  {
    try
    {
      ...
      if (!tile.classList.contains(dimension))
      {
        var mlCommon = tile.parentNode._mlCommon;
        amxNode.setAttributeResolvedValue("ignoreDrag", true);
        id = id.replace(/:/g, "\\:");
        var masonryLayoutAMXN = adf.mf.api.amx.AmxNode.getAmxNodeForElement(tile.parentNode);
        masonryLayoutAMXN.setAttributeResolvedValue("resizeAmxNode", amxNode);
        mlCommon.resizeTile("#" + id, dimension);        
      }
      ...
      masonryLayoutItem.superclass.refresh.call(this, amxNode, attributeChanges, descendentChanges);
    }
    catch (problem)
    {
      console.log(problem);
